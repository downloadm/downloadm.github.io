...
*/
extern unsigned xlate_string PROTO((const char *string,darray_ptr phone));
extern void say_string PROTO((char *s));
extern char *concat_args PROTO((int argc,char *argv[]));
extern int suspect_word PROTO((const char *s,int n));
extern void say_phones PROTO((char *phone,int len, int verbose));
extern unsigned spell_out PROTO((const char *word,int n,darray_ptr phone));
